...
| --- | --- | --- | --- | --- |
| `DIMENSION_CURVE` / `DIMENSION_CURVE_TERMINATOR` | Visible dimension lines and arrows | Helper display only | Not started | Current helper geometry is FreeCAD-native, not AP242 presentation PMI. |
| `PRESENTATION_SIZE` | Presentation text/symbol sizing | Helper display only | Not started | Text size is still a known GUI/display issue. |
...
| --- | --- | --- | --- |
| Short term | Continue using helper objects for rapid iteration | Active | Dimensions and datums still use helper display objects while their custom view providers are developed. |
...
| Medium term | Avoid storing derived visual geometry when it can be regenerated | In progress | FCF and datum-target geometry are regenerated by their owning view providers; dimensions and datums remain. |
| Long term | Replace helper display objects with custom FreeCAD view-provider rendering | In progress | FCFs and datum targets have single-tree-item implementations; dimensions and datums remain. |
| Long term | Draw lines, boxes, symbols, leaders, and text through the owning PMI view provider | In progress | FCF and datum-target view providers draw their visible annotation directly with Coin3D. |
| Long term | Allow manual annotation placement in the 3D view | In progress | Double-clicking an FCF enters a planar drag mode and stores the resulting annotation origin; GUI validation is required. |
