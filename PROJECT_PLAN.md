...
| Add semantic circular/rectangular/area datum target definition/export | In progress | Circular and rectangular datum targets are modeled, validated, displayed, and exported through AP242 placed target features. Arbitrary area targets need a real sketch/boundary-on-datum workflow before export; do not use fake construction faces as the intended user workflow. |
...
| 2 | Circular, rectangular, and area datum targets | In progress | Circular and rectangular target areas are implemented and passed GUI export testing. Rectangular target orientation is currently inferred from the datum plane frame; explicit user-controlled in-plane orientation remains future work. Arbitrary area targets need a sketch-boundary definition workflow before AP242 export is revisited. |
| 3 | FCF modifiers and richer tolerance zones | In progress | Material-condition modifier, projected tolerance zone, and unequally disposed profile fields now exist in the FreeCAD model, display in FCF cells, and validate conservatively. MMC/LMC export through AP242 `GEOMETRIC_TOLERANCE_WITH_MODIFIERS` is covered, and projected-zone height exports through `PROJECTED_ZONE_DEFINITION` for position FCFs. Unequal disposition export and richer zone definition remain future work. |
...
### Future Arbitrary Area Datum Target Workflow

Arbitrary area datum targets should be implemented as real bounded areas on the
datum feature, not as fake 3D construction faces. Intended workflow:

1. User selects an existing MBD datum feature.
2. User chooses `Area` in the datum target command.
3. The workbench creates or asks for a sketch mapped to the datum feature face.
4. User draws one closed profile that defines the target boundary.
5. The workbench validates that the profile is closed, planar, non-self-intersecting, nonzero area, and lies on the datum feature surface.
6. The datum target stores the sketch/profile reference plus resolved boundary geometry, center, area, and surface attachment signature.
7. The view provider displays the bounded target area, leader, and target identifier on the datum surface.
8. AP242 export writes an area-style datum target only after the bounded-area STEP representation is selected and verified.
...
| `DATUM_TARGET` | Datum target | Partial | Partial | Point, line, circle, and rectangle targets are modeled and exported. Arbitrary area targets require a future bounded-sketch workflow on the datum face. |
| `PLACED_DATUM_TARGET_FEATURE` | Placed target feature | Partial | Partial | Point, line, circle, and rectangle targets export with placement and size parameters where applicable. Arbitrary area target export remains deferred until target boundaries are represented semantically. |
...
| `GEOMETRIC_TOLERANCE_WITH_MODIFIERS` | FCF modifiers | Partial | Partial | Profile all-over, MMC/LMC material-condition modifiers, and projected-zone definitions export through AP242 modifier/zone paths. Unequally disposed profile is modeled/displayed/validated but not yet exported as AP242 modifier entities. |
...
| `PROJECTED_ZONE_DEFINITION` | Projected tolerance zone | Partial | Partial | Position FCF projected-zone height exports through a post-write AP242 entity pass that references the base `TOLERANCE_ZONE`; broader projected-zone cases remain future work. |
| `RUNOUT_ZONE_DEFINITION` | Runout zone definition | Partial | Partial | Runout FCFs are modeled and exported, but explicit AP242 runout-zone geometry remains a future refinement. |
| `RUNOUT_ZONE_ORIENTATION` | Runout zone orientation | Partial | Partial | Runout FCFs are modeled and exported, but explicit AP242 runout-zone orientation remains a future refinement. |
...
| Draughting/presentation annotation entities | Visible PMI graphics | View-provider display only | Future | Defer AP242 presentation export until semantic export and display-layout properties stabilize. |
...
| Short term | Organize helper objects so the model tree remains usable | Done | `MBD PMI` group organizes semantic owners; helper-object grouping remains only for legacy cleanup and fallback paths. |
...
| FCF material-condition and projected-zone AP242 export | Done | Headless regression confirms an MMC position FCF exports `GEOMETRIC_TOLERANCE_WITH_MODIFIERS((.MAXIMUM_MATERIAL_REQUIREMENT.))` and `PROJECTED_ZONE_DEFINITION`. |
| FCF unequal-disposition export warning | Done | Exporter warns when an FCF contains `UnequallyDisposedZone` because that semantic is modeled/displayed/validated but not yet written to AP242. |
...
| Projected-zone AP242 export and unequal-disposition warning | Passed | GUI export `MBDTest01_BM.step` reported `Added 1 AP242 projected zone definitions after STEP write`; STEP text contains `PROJECTED_ZONE_DEFINITION` and the expected material modifier. The same export warned that the unequal-disposition offset is not yet included in AP242 export. |
...
| Comment all the code cleanly to make it sustainable | Ongoing | Initial maintainability pass removed obsolete timing diagnostics and documented the view-provider migration, post-write AP242 dimension paths, datum-target geometry, and validation rules. Broader cleanup should continue as features stabilize. |
